...
class MistralModel(BaseModel):
    """Implementation of Mistral models using Hugging Face"""
    ...
    def _format_chat_prompt(self, messages: List[Dict[str, str]]) -> str:
        ...
        # Mistral uses a specific format for chat
        for message in messages:
            role = message["role"].lower()
            content = message["content"]
            
            if role == "system":
                # System messages go at the beginning as a user message with special format
                formatted_messages.append(f"[INST] {content} [/INST]")
            elif role == "user":
                # Check if this is the first user message after a system or assistant message
                if not formatted_messages or "assistant" in formatted_messages[-1].lower():
                    formatted_messages.append(f"[INST] {content} [/INST]")
                else:
                    # Continuation of user messages
        ...
        # Ensure the last message requests a response if it's not from the assistant
        if not formatted_messages or "assistant" in formatted_messages[-1].lower():
            # Last message is from assistant, no need to add a prompt
            pass
        else:
            # Add empty assistant turn to prompt for response
