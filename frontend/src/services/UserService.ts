...
export class UserService {
   ...
   */
  static async getMyStarredRepositories(
    page: number = 1, 
    limit: number = 10
    ...
    try {
      ...
      const response = await api.get(`/accounts/me/starred?${params.toString()}`);
      return {
        repositories: response.data.repositories || [],
        pagination: response.data.pagination
      };
    } catch (error) {
      console.error('Error fetching my starred repositories:', error);
      throw error;
    }
  }
