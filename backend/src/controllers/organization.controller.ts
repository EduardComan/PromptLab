...
export const deleteOrganization = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Delete organization memberships first (cascade delete is not enabled by default)
...
// Get popular public organizations
export const getPopularOrganizations = async (req: Request, res: Response): Promise<void> => {
  try {
    const { limit = 10 } = req.query;
    const limitNum = parseInt(limit as string) || 10;

    // Get organizations with repositories that have the most stars
    ...
    // Calculate total stars for each organization
    const orgsWithStarCount = organizations.map(org => {
      const totalStars = org.repositories.reduce((sum, repo) => sum + repo.stars.length, 0);
      const { repositories, ...orgWithoutRepos } = org;
      return {
        ...orgWithoutRepos,
    ...
    orgsWithStarCount.sort((a, b) => b.total_stars - a.total_stars);

    res.status(200).json({ organizations: orgsWithStarCount });
  } catch (error: unknown) {
    logger.error('Error fetching popular organizations:', error);
    res.status(500).json({
      message: 'Error fetching popular organizations',
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : undefined,
    });
  }
};
...
export const uploadOrganizationLogo = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const fileName = `org_logo_${id}_${Date.now()}${fileExtension}`;
...
export const getOrganizationRepositories = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Check if user is a member to see private repos
